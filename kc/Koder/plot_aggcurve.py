...
def merge_files(file_list):
    ...
    for file in file_list:
        ...
        for chrom in data:
            ...
            adjusted_time = [t + offset for t in data[chrom]["time"]]
...
# ===============================
if len(sys.argv) < 3:
    print("Usage: python plot_aggcurve.py <labname> <antibody> [well]")
    sys.exit(1)

lab = sys.argv[1]
antibody = sys.argv[2]
specific_well = sys.argv[3] if len(sys.argv) >= 4 else None
...
# sekunder → timmar
time = [(t - time[0]) / 3600 for t in time]
...
# TITEL
# ===============================
concs = selected_rows["conc_uM"].unique()
if len(concs) == 1:
    title = f"{antibody} ({concs[0]} µM)"
else:
    title = f"{antibody} ({min(concs)}–{max(concs)} µM)"
...
# PLOT FULL RANGE
# ===============================
print("Showing full time range. Close plot window to continue...")
...
if cutoff_input.strip() != "":
    try:
        ...
        print(f"Replotting up to {cutoff} hours...")

        valid_indices = [i for i, t in enumerate(time) if t <= cutoff]
        time_cut = [time[i] for i in valid_indices]
        ...
        for conc in sorted_concs:
            ...
            for idx, w in enumerate(wells_in_group):
                if w in wells_dict:
                    data_cut = [wells_dict[w][i] for i in valid_indices]
